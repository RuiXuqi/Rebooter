...
|  1.2.0  | `FermiumRegistryAPI.isModPresent(String)`<br>`FermiumRegistryAPI.registerAnnotatedMixinConfig(Class<?>, Object)` (removed in 1.3.0)<br>`MixinConfig.SubInstance` (removed in 1.3.0)<br>`MixinConfig.EarlyMixin.name()` (removed in 1.3.0)<br>`MixinConfig.LateMixin.name()` (removed in 1.3.0)<br>`MixinConfig.CompatHandling.modid()`<br>`MixinConfig.CompatHandling.desired()`<br>`MixinConfig.CompatHandling.disableMixin()`<br>`MixinConfig.CompatHandling.reason()`<br>`MixinConfig.CompatHandlingContainer.value()` |
