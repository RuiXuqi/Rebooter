...
Rebooter aims to provide API and ABI compatibility through 1.4.1, including APIs introduced in 1.2.0 and removed from
later releases.
...
|  1.2.0  | `FermiumRegistryAPI.isModPresent(String)`<br>`FermiumRegistryAPI.registerAnnotatedMixinConfig(Class<?>, Object)` (deprecated since 1.3.0)<br>`MixinConfig.SubInstance` (deprecated since 1.3.0)<br>`MixinConfig.EarlyMixin.name()` (deprecated since 1.3.0)<br>`MixinConfig.LateMixin.name()` (deprecated since 1.3.0)<br>`MixinConfig.CompatHandling.modid()`<br>`MixinConfig.CompatHandling.desired()`<br>`MixinConfig.CompatHandling.disableMixin()`<br>`MixinConfig.CompatHandling.reason()`<br>`MixinConfig.CompatHandlingContainer.value()` |
|  1.3.0  | `MixinConfig.name()`<br>`MixinConfig.MixinToggle.earlyMixin()`<br>`MixinConfig.MixinToggle.lateMixin()`<br>`MixinConfig.MixinToggle.defaultValue()`                                                                                                                                                                                                                                                                                                                                                                                               |
